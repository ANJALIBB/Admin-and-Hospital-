package com.anj.adminhospital.controller;

import java.util.List;
import java.util.Scanner;

import com.anj.adminhospital.dao.Admindao;
import com.anj.adminhospital.dao.Hospitaldao;
...
public class AdminHospitalController {
	...
	public static void findHospitalsByAdminPhone$Pasword() {
		...
		long phone = s.nextLong();
		String password = s.next();
		List<Hospital> hospitals = hospitalDao.findHospitalsByPhoneNPass(phone, password);
		if (hospitals.size() > 0) {
			for (Hospital h : hospitals) {
				System.out.println("Hospital Id:" + h.getId());
				System.out.println("Hospital name:" + h.getName());
